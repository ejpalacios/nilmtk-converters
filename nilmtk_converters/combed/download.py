            ...
            for floor in floors:
                for measurement in MEASUREMENTS:
                    query_instance = query.format(
                        block_name, "Academic Building", load, str(floor), measurement
                    )
                    DATA_URL = join(
                        SMAP_URL, "backend/api/data/uuid/{}?starttime={}&endtime={}"
                    )
                    uuid = requests.post(UUID_URL, query_instance).json()[0]["uuid"]
                    data = requests.get(
                        DATA_URL.format(uuid, START_TIME, END_TIME)
                    ).json()[0]["Readings"]
                    ...
                    df.to_csv(
                        path_to_create + "/" + measurement + ".csv",
                        header=False,
                        index=False,
                    )
